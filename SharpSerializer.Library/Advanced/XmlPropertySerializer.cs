...
namespace Polenter.Serialization.Advanced
{
    ...
    public sealed class XmlPropertySerializer : PropertySerializer
    {
        ...
        /// <summary>
        /// spit out simple property children as attributes
        /// </summary>
        /// <param name="children">child properties</param>
        private void addStartPropertyAttributes(PropertyCollection children)
        {
            //loop through child properties and look for Simple types
            foreach (var child in
                from c in children
                where c is SimpleProperty
                select c as SimpleProperty)
            {
                //loop through all xml attribute attributes assigned to it
                foreach (var attrib in
                    from a in child.Attributes
                    where a is XMLAttributeAttribute
                    select a as XMLAttributeAttribute)
                {
                    //write them out as xml attributes
                    _writer.WriteAttribute(attrib.AttributeName, child.Value);
                }
            }
        }
        ...
        protected override void SerializeComplexProperty(PropertyTypeInfo<ComplexProperty> property)
        {
            ...
            addStartPropertyAttributes(property.Property.Properties);
